...
import {Service} from './list.models';

export const listAdapter = createEntityAdapter<Service>();

export interface ListState extends EntityState<Service> {
  items: Service[],
  loading: boolean;
  error: string;
}
...
export function reducer(state: ListState = initialState, action: fromActions.All) {
  switch (action.type) {
    ...
    case fromActions.Types.CREATE_SUCCESS: {
      return listAdapter.addOne(action.service, state);
    }
