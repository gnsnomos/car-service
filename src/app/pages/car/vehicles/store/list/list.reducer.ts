...
import {Vehicle} from './list.models';

export const listAdapter = createEntityAdapter<Vehicle>();

export interface ListState extends EntityState<Vehicle> {
  items: Vehicle[],
  loading: boolean;
  error: string;
}
...
export function reducer(state: ListState = initialState, action: fromActions.All) {
  switch (action.type) {
    ...
    case fromActions.Types.CREATE_SUCCESS: {
      return listAdapter.addOne(action.vehicle, state);
    }
